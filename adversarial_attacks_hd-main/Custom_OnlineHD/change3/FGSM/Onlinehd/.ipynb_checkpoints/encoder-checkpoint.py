...
class Encoder(object):
    ...
    '''
#     def __init__(self, features : int, dim : int = 4000, target_dim : int = 3000):
    def __init__(self, features : int, dim : int = 4000):
        self.dim = dim
        self.features = features
        self.basis = torch.randn(self.dim, self.features)
        self.base = torch.empty(self.dim).uniform_(0.0, 1).round()
...
#            it should be changed for backpropagation
...
#             h[i:i+bsize].cos_().mul_(temp.sin_())
            ...
            h[i:i+bsize] = h[i:i+bsize].mul(temp.clone().detach())
        return h
