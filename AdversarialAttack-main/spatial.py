...
def cos_cdist(x1 : torch.Tensor, x2 : torch.Tensor, eps : float = 1e-8):
    ...
    norms1 = torch.unsqueeze(torch.norm(x1, dim=1), dim=1).max(eps)
    norms2 = torch.unsqueeze(torch.norm(x2, dim=1), dim=0).max(eps)
    # norms1 = x1.norm(dim=1).unsqueeze_(1).max(eps)
    # norms2 = x2.norm(dim=1).unsqueeze_(0).max(eps)
    #cdist = x1 @ x2.T
    cdist = torch.real(torch.matmul(x1,x2.T.conj()))
    cdist = torch.div(cdist, norms1)
    cdist = torch.div(cdist, norms2)
