...
class Encoder(object):
    ...
    '''
    def __init__(self, features : int, dim : int = 4000,randgen=None):
        self.dim = dim
        ...
        if randgen is not None:
            self.basis=np.array(randgen.approx(5,50,self.dim*self.features).reshape((self.dim,self.features)).astype(np.float))
            self.basis=torch.from_numpy(self.basis)
            self.basis=self.basis.float()
    ...
    def __call__(self, x : torch.Tensor):
        ...
        h = torch.empty(n, self.dim, device=x.device, dtype=torch.cfloat)
        temp = torch.empty(bsize, self.dim, device=x.device, dtype=torch.float)

        # we need batches to remove memory usage
        for i in range(0, n, bsize):
            
            temp = torch.matmul(x[i:i+bsize], self.basis.T)
            temp = torch.add(temp, self.base)
            h[i:i+bsize] = torch.mul(temp, 1j)
            #h[i:i+bsize].exp_()
            h[i:i+bsize] = h[i:i+bsize].exp()
        ...
        #     h[i:i+bsize] = h[i:i+bsize].clone().cos()
        #     tmp2 = temp.sin()
        #     h[i:i+bsize] = h[i:i+bsize].mul(tmp2.clone().detach())
