...
class GenAttack(object):
    def __init__(self, model, num_labels, threshold = 0.95, device = 'cpu'):
    ...
    def get_fitness(self, population, target):
        ...
        pop_preds = self.model.probabilities(population.reshape(population.size(0), -1)) # .reshape(population.size(0), 1, 28, 28)
    ...
    def attack(self, x, target, delta, alpha, p, N, G):
        ...
        for g in range(G):
            ...
            # print(target, self.model(Pcurrent.reshape(Pcurrent.shape[0], -1)))
            if target in self.model(Pcurrent.reshape(Pcurrent.shape[0], -1)): # .reshape(Pcurrent.shape[0], -1, 28, 28) .argmax(1)
...
def make_GA(model, delta, alpha, x_test, y_test, display=True):
    if alpha==0:
        return x_test, y_test
    classes = y_test.unique().size(0)
    attacker = GenAttack(model, classes)
    ...
    for i in tqdm(range(x_test.size(0)), disable=(not display)):
        temp = attacker.attack(x_test[i].unsqueeze(0), targets[i], delta, alpha, p, N, G)
        if temp[1]:
            # temp_input = scaler.transform(temp[0].clone().reshape(-1, 784))
            # temp_input = torch.from_numpy(temp_input).float().cuda()
            test_data.append(
                torch.from_numpy(temp[0][(model(temp[0].cpu()) == targets[i]).nonzero()[0].item()].numpy())) # .argmax(1)
            success.append(
                torch.from_numpy(temp[0][(model(temp[0].cpu()) == targets[i]).nonzero()[0].item()].numpy())) # .argmax(1)
        else:
