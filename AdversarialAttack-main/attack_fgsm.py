...
def attack(model, x, x_test, y_test, epsilons, classes, criterion, N_VAL_SAMPLES, device, input_dim=(28, 28), model_type='cnn', display=True):
    ...
    for eps in epsilons:
        ...
        ### NOTE: IT WOULD BE MORE EFFICIENT TO ITERATE ONLY ONCE THROUGH THE DATA AND PERFORM ALL THE ATTACKS
            ...
            y_pred = model(x_origin.reshape(-1, flatten_input_dim))
            ...
            output = model.encode(x_origin.reshape(-1, flatten_input_dim))
            loss = criterion(output, model.model, label) # .cuda()
            grad = torch.autograd.grad(outputs=loss, inputs=x_origin)[0]

            perturbed_x, _ = fgsm_attack(x_origin, epsilon=eps, gradient=grad)
            perturbed_output = model(perturbed_x.reshape(-1, flatten_input_dim))   
            ...
            if model_type == 'cnn':
                attack[j] = perturbed_x.reshape(*input_dim)
            else:
                attack[j] = perturbed_x.reshape(input_dim)
            labels[j] = y_target
        ...
        if model_type == 'cnn' and display:
            plt.imshow(perturbed_x.detach().cpu().numpy().reshape(*input_dim), cmap='gray')
            plt.colorbar()
            plt.show()
